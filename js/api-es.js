...
async function genEmail() {
    try {
        ...
        await refreshMail();
        showError('¡Nueva dirección de correo temporal creada exitosamente!', true);
    } catch (error) {
        console.error('Error generating email:', error);
        showError(`Error al crear nueva dirección de correo temporal: ${error.message}`);
        clearStoredData();
        sessionId = '';
...
async function copyEmail() {
    ...
    if (!email) {
        showError('No hay dirección de correo para copiar');
        return;
    }

    try {
        await navigator.clipboard.writeText(email);
        showError('¡Dirección de correo copiada al portapapeles!', true);
    } catch (error) {
        showError('Error al copiar la dirección de correo: ' + error.message);
    }
}
...
async function refreshMail() {
    if (!currentEmail || !sessionId) {
        showError('No hay sesión activa de correo electrónico');
        return;
    }
    ...
    try {
        ...
        showError('Error al obtener correos');
...
function updateEmailTable(emails) {
    ...
    if (!emails || emails.length === 0) {
        ...
        emptyRow.innerHTML = '<td colspan="5" class="text-center py-8 text-gray-500 dark:text-gray-400">No hay correos</td>';
    ...
    emails.forEach(email => {
        ...
        row.innerHTML = `
            ...
            <td class="py-4 px-6 text-sm text-gray-600 dark:text-gray-300">${new Date(email.mail_timestamp * 1000).toLocaleString('es-ES')}</td>
            <td class="py-4 px-6">
...
async function viewEmail(id) {
    ...
    try {
        ...
        showError('Error al cargar el contenido del correo');
    } finally {
        setLoading(false);
    }
}
        ...
        <div class="email-modal-content bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-90vh overflow-hidden">
            <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white truncate max-w-md">${email.mail_subject || '(Sin asunto)'}</h2>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Detalles del correo</p>
                    </div>
                </div>
            ...
            <div class="p-6 overflow-y-auto max-h-96">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">Remitente:</span>
                        </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">Hora de recepción:</span>
                        </div>
                        <p class="text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 rounded-lg p-3">${new Date(email.mail_timestamp * 1000).toLocaleString('es-ES')}</p>
                    </div>
                </div>
                ...
                <div class="space-y-3 mb-6">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-semibold text-gray-900 dark:text-white">Contenido del correo:</span>
                    </div>
                    <div class="email-body bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600 max-h-80 overflow-y-auto">
                        <div class="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300">
                            ${email.mail_body || '<p class="text-gray-500 dark:text-gray-400 italic">Este correo no contiene texto</p>'}
                        </div>
                    </div>
                </div>
                ...
                ${email.mail_attachments?.length ? `
                    <div class="space-y-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">Adjuntos (${email.mail_attachments.length}):</span>
                        </div>
...
async function downloadAttachment(emailId, filename) {
    ...
    try {
        showError('Descargando adjunto...', true);
        ...
        a.remove();
        showError('¡Adjunto descargado exitosamente!', true);
    } catch (error) {
        console.error('Error downloading attachment:', error);
        showError('Error al descargar adjunto');
    }
}
...
async function deleteEmail(id) {
    ...
    try {
        ...
        refreshMail();
        showError('¡Correo eliminado exitosamente!', true);
    } catch (error) {
        console.error('Error deleting email:', error);
        showError('Error al eliminar correo');
    }
}
